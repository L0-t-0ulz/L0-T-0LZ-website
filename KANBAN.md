...
- [x] **F4 · Idle ATTRACT MODE** ✅ live — after ~18s idle, a self-playing arcade demo: wandering eyes, auto-spinning garment cycling fabrics, streaming telemetry. Any input exits. `⚡` `M` `Novelty`
- [x] **F6 · Shareable "MY CALIBRATION" card** ✅ live — a generative per-visitor optic-sigil rendered to a 1080×1350 share image with the vault code → download / native share. Built-in virality. `⚡` `L` `Discovery`
